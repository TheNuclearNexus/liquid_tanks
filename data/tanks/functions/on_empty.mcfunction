...
execute if score @s ES_stored matches ..0 if entity @s[scores={ES_ID=6}] run tag @s remove ES_CM_T
execute if score @s ES_stored matches ..0 if entity @s[scores={ES_ID=6}] run tag @s remove ES_CM_F
execute at @s if score @s ES_stored matches ..0 if entity @s[scores={ES_ID=6}] run setblock ~ ~ ~ hopper[enabled=true]{TransferCooldown:1,CustomName:"{\"italic\":false,\"color\":\"aqua\",\"text\":\"Liquid Tank\"}"}
