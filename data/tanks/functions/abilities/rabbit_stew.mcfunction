execute if score @s ES_stored matches 2.. if entity @e[distance=..1,tag=!emptied,type=item,nbt={Item:{id:"minecraft:bowl",Count:1b}}] run tag @s add fillRabBowl
execute as @s[tag=fillRabBowl] run data merge entity @e[limit=1,distance=..1,tag=!filled,type=item,nbt={Item:{id:"minecraft:bowl",Count:1b}}] {Tags:["filled"],Item:{id:"minecraft:rabbit_stew"}}
execute as @s[tag=fillRabBowl] run scoreboard players remove @s ES_stored 2
execute as @s[tag=fillRabBowl] run tag @s remove fillRabBowl
...
execute if score @s[scores={ES_ID=8}] ES_temp matches 1.. run scoreboard players remove @s ES_temp 1
execute if score @s[scores={ES_ID=8}] ES_temp matches 0 run scoreboard players reset @s ES_temp
