...
execute unless block ~ ~ ~ hopper{Items:[{Slot:2b}]} if entity @s[scores={ES_ID=1},tag=ES_FC_F,tag=!ES_FC_T] run tag @s add swapT
execute unless block ~ ~ ~ hopper{Items:[{Slot:2b}]} if entity @s[scores={ES_ID=1},tag=ES_FC_T,tag=!ES_FC_F] run tag @s add swapF

execute if entity @s[scores={ES_ID=1},tag=swapT] run playsound minecraft:ui.button.click master @a[distance=..5]
execute if entity @s[scores={ES_ID=1},tag=swapF] run playsound minecraft:ui.button.click master @a[distance=..5]

execute if entity @s[scores={ES_ID=1},tag=swapT] run tag @s add ES_FC_T
execute if entity @s[scores={ES_ID=1},tag=swapF] run tag @s add ES_FC_F

execute if entity @s[scores={ES_ID=1},tag=swapT] run tag @s remove ES_FC_F
execute if entity @s[scores={ES_ID=1},tag=swapF] run tag @s remove ES_FC_T

execute if entity @s[scores={ES_ID=1},tag=swapT] run tag @s remove swapT
execute if entity @s[scores={ES_ID=1},tag=swapF] run tag @s remove swapF
...
execute if entity @s[scores={ES_ID=1},tag=ES_FC_T] if score @s ES_stored matches 2.. unless score @s ES_temp matches 1.. if block ~ ~-1 ~ cauldron run tag @s add fillCauldron
...
execute if entity @s[scores={ES_ID=1},tag=ES_FC_F] if score @s ES_stored matches 2.. unless score @s ES_temp matches 1.. if block ~ ~-1 ~ air positioned ~ ~-1.6 ~ if entity @a[distance=..0.5] run tag @s add extinguish
